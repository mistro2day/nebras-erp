...
@Component({
  ...
  template: `
    <div class="page" dir="rtl">
      <nb-page-header title="الشعب الدراسية" subtitle="شعب كل صف مع الطاقة الاستيعابية والفترة والنوع.">
        <button class="nb-btn-secondary" (click)="load()">تحديث</button>
        <button class="nb-btn-primary" (click)="adding.set(!adding())" [disabled]="grades().length === 0">{{ adding() ? 'إغلاق' : 'إضافة شعبة' }}</button>
      </nb-page-header>
      ...
      @if (adding()) {
        <nb-panel title="شعبة جديدة" style="margin-bottom:16px">
          <div class="add-form">
            ...
            <div class="fld req"><label>اسم الشعبة</label><input [(ngModel)]="f.name" placeholder="مثال: شعبة (أ)" /></div>
      ...
      <nb-panel [flush]="true">
        <div class="tbl">
          <div class="tbl-head" style="grid-template-columns:1.3fr 1.3fr 0.7fr 0.8fr 0.9fr 1fr 0.8fr">
            <span>الشعبة</span><span>الصف</span><span>الرمز</span><span>السعة</span><span>النوع</span><span>الفترة</span><span>إجراءات</span>
          </div>
...
export class AcademicSectionsComponent implements OnInit {
  ...
  remove(s: any): void {
    const data: ConfirmDialogData = { title: 'حذف الشعبة', message: `حذف «${s.name}»؟`, color: 'warn' };
