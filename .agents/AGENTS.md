...
- **ZERO PROBLEMS & THOROUGH TESTING BEFORE COMMIT/PUSH (حظر الرفع بوجود مشاكل أو أخطاء برمجية - الاختبار أولاً ثم الرفع)**:
  It is **STRICTLY PROHIBITED** to commit or push any changes to Git if there are syntax errors, compile issues, lint errors, or active Problems.
...
- **MANDATORY GIT PUSH (الإلزام الصارم بعمل git push دائماً)**:
  After committing any changes, it is **STRICTLY MANDATORY** to push changes to the remote repository (`git push origin <branch>`). Never consider any task finished without pushing to remote.
