...
class DocumentStorageService:
    ...
    def upload_file(tenant_id, folder_id, title, file_obj, owner_id):
        ...
        if folder_id:
            try:
                folder = DocumentFolder.objects.get(tenant_id=tenant_id, id=folder_id)
                # فحص الصلاحية هنا
            except DocumentFolder.DoesNotExist:
                raise ValidationError("المجلد المحدد غير موجود.")

        # تخزين الملف في مجلد التخزين المخصص للمستأجر
        relative_path = f"tenant_{tenant_id}/dms/{uuid.uuid4()}_{file_obj.name}"
    ...
    def add_new_version(tenant_id, document_id, file_obj, author_id, change_log, is_major=False):
        ...
        """
        try:
            document = Document.objects.get(tenant_id=tenant_id, id=document_id)
        ...
        if document.is_locked:
            ...
            if lock and str(lock.locked_by) != str(author_id):
...
class DocumentLockService:
    ...
    def unlock_document(tenant_id, document_id, user_id):
        ...
        lock = DocumentLock.objects.filter(document=document).first()
        if lock and str(lock.locked_by) != str(user_id):
