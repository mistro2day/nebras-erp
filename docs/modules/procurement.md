...
### قاعدة الملكية: الأبعاد المالية مِلك المالية

**حساب الموازنة ومركز التكلفة بُعدان ماليان تملكهما وحدة المالية، والمشتريات تستهلكهما فقط.**
لا تُنشئ المشتريات ولا تُعدّل مراكز التكلفة إطلاقاً؛ تُدار حصراً من `/finance/cost-centers`.

هذا مطابق لنمط الأنظمة المرجعية:

| النظام | التسمية | الوحدة المالكة | موضع الاستهلاك |
| :--- | :--- | :--- | :--- |
| **Odoo** | الحساب التحليلي (Analytic Account) والتوزيع التحليلي | المحاسبة | سطر طلب/أمر الشراء |
| **Dynamics 365** | مركز التكلفة كبُعد مالي (Financial Dimension) | دفتر الأستاذ العام | قسم «الأبعاد المالية» في سطر الطلب |
| **نبراس** | `finance.CostCenter` + `finance.ChartOfAccount` | المالية | سطر بند طلب الشراء |

يتحقّق الخادم من صلاحية البُعدين عند إنشاء الطلب (`ChartOfAccount.objects.get` و
`CostCenter.objects.get`)، ويُستهلك رصيد الموازنة فعلياً عند **إصدار أمر الشراء**.

---

## 8. آخر حلقة: فاتورة المورّد والترحيل المحاسبي (Vendor Invoice → GL)

بحسب نمط **Odoo** (Vendor Bill) و**Dynamics 365** (Vendor invoice): **أمر الشراء لا يُرحَّل
محاسبياً** — هو التزام/ارتباط (Encumbrance) يستهلك الموازنة فقط. **فاتورة المورّد** هي التي
تُرحَّل في دفتر الأستاذ.

* `POST /api/v1/procurement/orders/{id}/post-invoice/` — body: `{invoice_number, invoice_date?}`

القيد المتولّد آلياً:

| الطرف | الحساب | المصدر |
| :--- | :--- | :--- |
| **مدين** | حساب الموازنة لكل بند (مصروف) | `PurchaseOrderItem.budget_account_id` |
| **دائن** | ذمم الموردين الدائنة | `PurchaseSettings.payable_gl_account_id` (افتراضي: رمز `2101`) |

يُرحَّل عبر `PostingService.post_journal_entry` فتتولّد قيود أستاذ (Ledger Entries) غير قابلة
للتغيير، ويصبح أمر الشراء `completed` مع حفظ `journal_entry_id` و`vendor_invoice_number`.

### دورة كاملة مُتحقَّق منها بالمحاكاة

| # | المرحلة | النتيجة |
| :--- | :--- | :--- |
| 1 | إنشاء طلب شراء | `PR-…` حالة `draft` — تقديري 5,000 |
| 2 | الاعتماد | `approved` |
| 3 | توليد RFQ | `RFQ-…` منشور + الطلب `rfq_created` |
| 4 | عرض سعر المورّد | 4,800 — مدة توريد 5 أيام |
| 5 | الترسية | `PO-…` مسودة 4,800 + البنود تحمل الحساب ومركز التكلفة |
| 6 | إصدار أمر الشراء | استهلاك الموازنة: 70,000 → **74,800** |
| 7 | ترحيل فاتورة المورّد | قيد متوازن: مدين 5200 = دائن 2101 = 4,800 → PO `completed` |

### نواقص معروفة (Backlog)

* **إدخال عروض الأسعار**: لا توجد شاشة لتسجيل عرض المورّد؛ الترسية جاهزة لكنها تتطلب عروضاً.
* **إشعار الاستلام (GRN)**: لا يوجد؛ الفاتورة تُرحَّل مباشرة بعد الإصدار دون مطابقة ثلاثية.
* **الدفع**: سداد ذمم المورّد (مدين 2101 / دائن البنك) غير منفّذ بعد.
