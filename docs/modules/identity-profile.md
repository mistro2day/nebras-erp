...
- **`Role` & `RolePermission`** (`apps/identity/domain/models.py`):
  - إدارة الأدوار المخصصة والنظامية (`SYSTEM_ROLES`) مع الربط بالمستأجر الفعّال `tenant_id`.
  - الأدوار الإدارية المعتمدة حديثاً:
    * **المحاسب المالي (`accountant`)**: إدارة السندات، الفواتير، الاستحقاقات، الخزائن النقدية، والبنوك والتقارير المالية.
    * **مسجل الطلاب والقبول (`registrar`)**: شؤون القبول والملفات الأكاديمية والطلابية والحضور والنماذج.
    * **مسؤول الموارد البشرية (`hr_officer`)**: إدارة الموظفين، مسيرات الرواتب واعتمادها، وتفعيل الحسابات.
  - حسابات المستخدمين الافتراضية المجهزة في النظام (Default Seed Users):
    * المحاسب: عثمان دفع الله البشير (`accountant@nebras.sd` / كلمة المرور الافتراضية: `Nebras@Password2026`).
    * المسجل: نزار المجذوب بابكر (`registrar@nebras.sd` / كلمة المرور الافتراضية: `Nebras@Password2026`).
    * مسؤول الموارد البشرية: إخلاص ميرغني التاج (`hr@nebras.sd` / كلمة المرور الافتراضية: `Nebras@Password2026`).
- **`UserSession`**:
