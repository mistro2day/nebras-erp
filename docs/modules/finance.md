...
* `/finance/reports` - التقارير المالية والختامية (الإيرادات حسب التاريخ، المصروفات حسب التاريخ، ميزان المراجعة، قائمة الدخل، مراكز التكلفة، والمركز المالي) مع بطاقات إحصاءات ممتدة على كامل عرض الصفحة وتصدير وطباعة A4 رسمية.
...
## 5.1 التهيئة الافتراضية لكل مستأجر (Auto-Provisioning)

يُهيَّأ النظام المالي تلقائياً لكل مستأجر جديد بمجرد إنشائه، فتصبح جاهزة للاستخدام دون أي إعداد يدوي:

* **دليل حسابات قياسي للمدارس (Standard School COA):** شجرة حسابات معروفة لأنظمة المؤسسات التعليمية تشمل حسابات مميّزة للقطاع مثل: *ذمم الطلاب المدينة (رسوم مستحقة)*، *رسوم دراسية مقبوضة مقدماً*، *رسوم النقل/الأنشطة/المقصف*، *رواتب الهيئة التدريسية*، *الحافلات المدرسية*، *التبرعات والمنح* — منظّمة تحت الفئات الخمس (أصول، خصوم، حقوق ملكية، إيرادات، مصروفات) بترقيم هرمي (1000–5307).
* **بيانات مرجعية:** أنواع الحسابات، التصنيفات، العملات، طرق الدفع، الضرائب (VAT/WHT)، والإعدادات المالية الافتراضية.

**آلية العمل:**
* المصدر الموحّد: `apps/finance/application/provisioning.py` (الدالة `provision_finance_defaults(tenant_id)` — idempotent).
* التفعيل التلقائي: إشارة `post_save` على نموذج `Tenant` في `apps/finance/signals.py` (مُسجّلة عبر `FinanceConfig.ready()`).
* تعبئة المستأجرين الحاليين: `python manage.py provision_finance [--tenant <id>]`.
* بذرة بيانات تجريبية كاملة (تطوير): `backend/seed_finance.py` تستدعي نفس الخدمة ثم تضيف بنوكاً ومراكز تكلفة وسنة مالية وقيوداً مرحّلة.

## 5.2 ربط اسم المحاسب المنشئ تلقائياً (Accountant & Creator User Resolution)

* **الربط التلقائي بمستخدم النظام (Automatic User Attribution):**
  * حفظ `created_by=request.user.id` تلقائياً عبر `BaseCRUDViewSet` في كافة السندات المالية (`Voucher`) والمعاملات (`FinancialTransaction`) وقيود اليومية (`JournalEntry`).
  * تزويد سيريالايزرات السندات والقيود بحقلي `created_by_name` و `accountant_name` مع التخزين المؤقت بالذاكرة لمنع أي استعلامات متكررة.
  * توافق هيكلي تام 100% مع قاعدة بيانات الإنتاج في AWS دون الحاجة لترحيلات هيكلية جديدة (`No Schema Migrations`).
* **شاشات ومخرجات الطباعة الرسمية:**
  * إظهار اسم المحاسب في شبكة تفاصيل السند ومصفوفة التوقيعات الرسمية في سندات الصرف والقبض (`voucher-print`).
  * إظهار اسم المحاسب في ترويسة سند القيد وخانة "إعداد المحاسب" في قيود اليومية (`journal-voucher-print`).
  * إظهار اسم المحاسب في خطوات المراجعة والتأكيد بمعالجات إنشاء السندات والقيود (`VoucherCreateModalComponent` و `JournalEntryCreateModalComponent`).
