...
* **منظومة الأختام الإدارية المتعددة (Multi-Department Official Seals):**
  1. **ختم الإدارة العامة والمدير (`stamp` / `stamp_url`):** صورة الختم العام المعتمد للمؤسسة ويستخدم في شهادات الطلاب، خطابات القبول، والقرارات الإدارية، واستمارات التسجيل.
  2. **ختم الإدارة المالية والخزينة (`stamp_finance` / `stamp_finance_url`):** صورة ختم القسم المالي، ويستخدم حصرياً في سندات القبض المالي، فواتير الرسوم، كشوفات الحساب المالية، سندات الصرف، قيود اليومية، وأوامر الشراء.
  3. **ختم الشؤون الأكاديمية والمتابعة (`stamp_academic` / `stamp_academic_url`):** صورة ختم شؤون الطلاب والمتابعة، ويستخدم في كشوفات الحضور والغياب، إشعارات المتابعة والتقييم، والإنذارات الأكاديمية.
* **نظام الاحتياط الذكي (Smart Fallback):** في حال عدم قيام المدرسة برفع ختم خاص للقسم المالي أو الأكاديمي، يقوم النظام تلقائياً باعتماد ختم الإدارة العامة (`stamp`)، لضمان عدم خروج أي مستند رسمي دون ختم معتمد.
* **الاسم الرسمي بالعربية (`name_ar` / `name`):** الاسم المعتمد للمدرسة (مثل: مدارس المورد الجديدة للتعليم الخاص الخاصة / مدارس المورد الجديدة للتعليم الخاص).
...
* يتم حفظ الشعار الرسمي (`tenants/logos/`) وكافة الأختام الإدارية الثلاثة (`tenants/stamps/`)، وكذلك وثائق ومستندات الطلاب (`{tenant}/students/{id}/attachments/`) مباشرة على S3 عند تفعيل متغير `USE_S3=true`.
* عند رفع الختم عبر واجهة المستخدم أو الـ API، يُنشئ النظام روابط سحابية دائمة ومباشرة وفائقة السرعة لتضمينها بسلاسة داخل مطبوعات المتصفح وملفات PDF.
* يتراجع النظام تلقائياً وبسلاسة إلى التخزين المحلي `FileSystemStorage` في بيئة التطوير المحلية في حال عدم توفر مفاتيح AWS، مما يضمن أمان واستمرارية عمل النظام دون أخطاء.
