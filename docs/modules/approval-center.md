...
يستخدم مركز الموافقات فئة `HasPermission` الديناميكية من `apps.identity.authorization` (تحقق حسب `UserRole`/`RolePermission` لكل مستأجر)، وليس صلاحيات Django الجديدة للتعليم الخاص الثابتة:
